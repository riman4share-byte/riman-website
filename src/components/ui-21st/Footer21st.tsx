...
export default function Footer21st() {
    ...
    <footer className="bg-onyx text-ivory pt-16 pb-10" aria-label="Riman footer">
      <div className="container mx-auto px-6 grid gap-12 md:grid-cols-3 text-start">
        ...
        <nav className="grid grid-cols-2 gap-3 text-sm" aria-label="Collections">
          {['Bridal', 'Evening', 'Rental', 'Alterations', 'Gallery', 'Book a Fitting'].map((label, i) => {
            const paths = ['/collection/bridal', '/collection/evening', '/collection/rental', '/alterations', '/gallery', '/appointment'];
            return (
              <Link key={paths[i]} to={paths[i]} className="text-white/70 hover:text-gold min-h-[44px] inline-flex items-center">
                {label}
              </Link>
            );
          })}
